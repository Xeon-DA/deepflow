...
### /proc 目录 {#inputs.procs.proc_dir_path}
...
### 同步间隔 {#inputs.procs.sync_interval}
...
### 最小活跃时间 {#inputs.procs.min_lifetime}
...
### Tag 提取 {#inputs.procs.tag_extraction}

#### 脚本命令 {#inputs.procs.tag_extraction.script_command}
...
#### 执行用户名 {#inputs.procs.tag_extraction.exec_username}
...
`inputs.procs.tag_extraction.exec_username`
...
### 进程匹配器 {#inputs.procs.process_matcher}
...
#### 匹配正则表达式 {#inputs.procs.process_matcher.match_regex}
...
#### 匹配类型 {#inputs.procs.process_matcher.match_type}
...
#### 匹配语言 {#inputs.procs.process_matcher.match_languages}
...
#### 匹配用户名 {#inputs.procs.process_matcher.match_usernames}
...
#### 仅匹配容器内的进程 {#inputs.procs.process_matcher.only_in_container}
...
#### 仅匹配有 Tag 的进程 {#inputs.procs.process_matcher.only_with_tag}
...
#### 匹配动作 {#inputs.procs.process_matcher.action}
...
#### 重命名 {#inputs.procs.process_matcher.rewrite_name}
...
### 符号表 {#inputs.ebpf.symbol_table}

#### Golang 特有 {#inputs.ebpf.symbol_table.golang_specific}

##### 进程匹配器 {#inputs.ebpf.symbol_table.golang_specific.process_matcher}
...
##### 刷新延迟时长 {#inputs.ebpf.symbol_table.java.refresh_defer_duration}
...
##### 符号表文件最大大小 {#inputs.ebpf.symbol_table.java.max_symbol_file_size}
...
### Socket 事件 {#inputs.ebpf.socket_event}

#### Uprobe {#inputs.ebpf.socket_event.uprobe}

##### Golang 进程 {#inputs.ebpf.socket_event.uprobe.golang_process}

The name of the process where each feature of ebpf uprobe takes effect,
which is configured using regular expressions

###### 进程匹配器 {#inputs.ebpf.socket_event.uprobe.golang_process.process_matcher}
...
###### 追踪超时时间 {#inputs.ebpf.socket_event.uprobe.golang_process.tracing_timeout}
...
##### TLS 请求 {#inputs.ebpf.socket_event.uprobe.tls_request}

###### OpenSSL 进程匹配器 {#inputs.ebpf.socket_event.uprobe.tls_request.openssl_process_matcher}
...
##### 黑名单 {#inputs.ebpf.socket_event.kprobe.blacklist}

###### 端口号 {#inputs.ebpf.socket_event.kprobe.blacklist.ports}
...
##### 白名单 {#inputs.ebpf.socket_event.kprobe.whitelist}

###### 比啊名单 {#inputs.ebpf.socket_event.kprobe.whitelist.port}
...
### IO 事件 {#inputs.ebpf.io_event}

#### 采集模式 {#inputs.ebpf.io_event.collect_mode}
...
`inputs.ebpf.io_event.collect_mode`
...
#### 最小耗时 {#inputs.ebpf.io_event.minimal_duration}
...
##### 进程匹配器 {#inputs.ebpf.profile.on_cpu.process_matcher}
...
Upgrade from old version: `static_config.ebpf.on-cpu-profile.regex`
...
##### 进程匹配器 {#inputs.ebpf.profile.off_cpu.process_matcher}
